...
@sealed
abstract class $EncryptionAlgorithmNativeAES {
  const $EncryptionAlgorithmNativeAES._();
...
Stream<EncryptionProgress> _encryptFile(
  ...
  Future<void>(() async {
    try {
      ...
      final tempFile = io.File(path.join(tempDir.path, path.basename('$sourceFile.tmp')));
      final encryptedFile = io.File(path.join(tempDir.path, path.basename('$sourceFile.enc')));
